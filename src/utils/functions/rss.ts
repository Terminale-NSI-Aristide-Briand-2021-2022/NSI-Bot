import { MessageActionRow, MessageButton, MessageComponentInteraction, MessageEmbed, TextChannel } from 'discord.js';
...
export const rss = async (client: Bot, last: number = -1) => {
  const notificationChannel = client.channels.cache.get(true ? `887745625637142570` : `887719893825388557`) as TextChannel;
  ...
  if (dateLastForum > last) {
    ...
    const embed = new MessageEmbed({
      ...
      color: authorName === 'Yann Bertrand' || authorName === 'Jérôme Cantaloube' ? 'RED' : 'DARK_PURPLE',
		...
		if (last === -1) {
			const checkChannel = client.channels.cache.get(`887745625637142570`) as TextChannel;
			const msg = await checkChannel.send({
				content: `Eh <@563749920683720709>, est ce que je dois envoyer ça ?`,
				embeds: [embed],
				components: [
